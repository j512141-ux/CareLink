 ...
 */
const GROUP_DICT = {
    '通知全家': [ {avatar:'assets/cute_grandpa_avatar_1775137046957.png', name:'阿公'}, {avatar:'assets/cute_daughter_avatar.png', name:'大女兒'}, {avatar:'assets/cute_son_avatar.png', name:'大兒子'}, {avatar:'assets/cute_daughter_avatar.png', name:'二媳婦'} ],
    '通知子女': [ {avatar:'assets/cute_daughter_avatar.png', name:'大女兒'}, {avatar:'assets/cute_son_avatar.png', name:'兒子'} ],
    '通知老友': [ {avatar:'assets/cute_uncle_avatar.png', name:'老李'}, {avatar:'assets/cute_aunt_avatar.png', name:'陳阿姨'}, {avatar:'assets/cute_uncle_avatar.png', name:'張大叔'}, {avatar:'assets/cute_aunt_avatar.png', name:'王太太'} ],
    '邀約通知': [ {avatar:'assets/cute_daughter_avatar.png', name:'大女兒'}, {avatar:'assets/cute_son_avatar.png', name:'大兒子'}, {avatar:'assets/cute_daughter_avatar.png', name:'二媳婦'} ]
};

let HELP_TASKS = [
    { id: 100, type: '想有人一起吃晚餐', who: '王爺爺', date: '週三', time: '早上 08:00', location: '青年公園', status: 'available', avatar: 'assets/cute_grandpa_avatar_1775137046957.png', helpers: [ { name: '大女兒', avatar: 'assets/cute_daughter_avatar.png' } ], createdBy: '王爺爺' },
    { id: 101, type: '陪散步', who: '老李', date: '4月8日', time: '下午 16:00', location: '青年公園', status: 'available', avatar: 'assets/cute_uncle_avatar.png', helpers: [], createdBy: '老李' },
    { id: 102, type: '代買豆漿', who: '陳阿姨', date: '4月8日', time: '早上 07:30', location: '巷口早餐店', status: 'ongoing', avatar: 'assets/cute_aunt_avatar.png', helpers: [ { name: '大兒子', avatar: 'assets/cute_son_avatar.png' } ], createdBy: '陳阿姨' },
    { id: 201, type: '幫忙買菜', who: '王爺爺', date: '週日', time: '中午 12:00', location: '巷口熱炒店', status: 'available', avatar: 'assets/cute_grandpa_avatar_1775137046957.png', helpers: [], createdBy: '王爺爺' },
    { id: 202, type: '幫我搬點東西', who: '王爺爺', date: '週五', time: '下午 14:00', location: '里民活動中心', status: 'available', avatar: 'assets/cute_grandpa_avatar_1775137046957.png', helpers: [], createdBy: '王爺爺' }
];
...
let currentStoryText = '阿北做事俐落有條理，善於應對客人，也是一位善良可靠的長者。';
...
function renderMySharePosts() {
    ...
    if (myList.children.length === 0) {
        const samplePosts = [
            {
                img: 'assets/post_gardening_grandpa_1776596447661.png',
                likes: 12,
                text: '今天早起整理了陽台的盆栽，看到新芽長出來，心裡真舒坦。'
            },
            {
                img: 'assets/post_home_dish_1776596569259.png',
                likes: 8,
                text: '煮了一鍋牛肉麵，香氣撲鼻，等孩子們回來一起吃。'
            },
            {
                img: 'assets/post_calligraphy_1776596936047.png',
                likes: 5,
...
function renderAllSharePosts() {
    ...
    if (allList.children.length === 0) { 
        const otherPosts = [
            {
                img: 'assets/others_post_market.png',
                name: '王奶奶',
                avatar: 'assets/cute_aunt_avatar.png',
                likes: 14,
                text: '早上去市場買了很漂亮的高麗菜 and 排骨，準備晚上煮湯。'
            },
            {
                img: 'assets/others_post_tea.png',
                name: '陳阿公',
                avatar: 'assets/cute_uncle_avatar.png',
                likes: 9,
                text: '今天跟老朋友去公園泡茶，非常開心！天氣也很好。'
            },
            {
                img: 'assets/others_post_yoga.png',
                name: '李阿姨',
                avatar: 'assets/cute_aunt_avatar.png',
                likes: 7,
            ...
            {
                img: 'assets/others_post_garden.png',
                name: '張伯伯',
                avatar: 'assets/cute_uncle_avatar.png',
                likes: 11,
                text: '今天整理了小菜園，收穫了不少。'
...
const PROFILE_FRIENDS = [
    { avatar: 'assets/cute_daughter_avatar.png', name: '大女兒' },
    { avatar: 'assets/cute_son_avatar.png', name: '大兒子' },
    { avatar: 'assets/cute_uncle_avatar.png', name: '老李' },
    { avatar: 'assets/cute_aunt_avatar.png', name: '陳阿姨' }
];
...
function switchPage(pageId) {
    ...
    tabs.forEach(tab => {
        ...
        if (img) {
            ...
            img.src = 'assets/tab_' + baseName + '.png';
        }
    });
    ...
    if(targetTab) {
        ...
        if (img) {
            const baseName = img.alt;
            img.src = 'assets/tab_' + baseName + '_active.png';
        }
    }
...
function submitNewAssist() {
    ...
    const newTask = {
        ...
        status: 'available',
        avatar: 'assets/cute_grandpa_avatar_1775137046957.png',
        helpers: [],
...
function startEditProfile() {
    ...
    const viewActions = document.getElementById('confirm-actions-view');
    const editActions = document.getElementById('confirm-actions-edit');
    if (!editArea || !viewArea || !skillInput || !storyInput || !viewActions || !editActions) return;
...
function saveEditedProfile() {
    ...
    const viewActions = document.getElementById('confirm-actions-view');
    const editActions = document.getElementById('confirm-actions-edit');
    if (!skillTextEl || !storyTextEl || !editArea || !viewArea || !skillInput || !storyInput || !viewActions || !editActions) return;
...
function cancelEditProfile() {
    ...
    const viewActions = document.getElementById('confirm-actions-view');
    const editActions = document.getElementById('confirm-actions-edit');
    if (!editArea || !viewArea || !viewActions || !editActions) return;
    editArea.style.display = 'none';
    viewArea.style.display = 'block';
    viewActions.style.display = 'block';
    editActions.style.display = 'none';
...
function updateCalendarDate() {
    ...
    if (lunarEl) {
        try {
            ...
            // Clean up the lunar string: keep only the part after '年' if present
            // On some mobile devices, it might return "2026(丙午)年三月十三"
            let lunarDate = lunarRaw;
            if (lunarRaw.includes('年')) {
                const parts = lunarRaw.split('年');
                lunarDate = parts[parts.length - 1]; // Take the last part (month and day)
            }
            
            // Further clean up: if it still contains numbers or Latin chars, it's likely a weird browser format
            // We only want the Chinese characters for month and day
            lunarDate = lunarDate.replace(/[0-9a-zA-Z\(\)\s-]/g, '').trim();
            
            // Ensure we don't accidentally remove everything
            if (!lunarDate || lunarDate.length < 2) {
                lunarDate = "三月十三"; // Fallback for demonstration if parsing fails completely
            }

            lunarEl.textContent = '農曆' + lunarDate;
        } catch (e) { 
            console.warn("Lunar error", e); 
            lunarEl.textContent = '農曆三月十三';
        }
    }
}
...
function acceptTask(id) {
    ...
    task.status = 'ongoing';
    task.helpers.push({ name: currentUserName, avatar: 'assets/cute_grandpa_avatar_1775137046957.png' });
    renderAssistMarket();
    showToast('你已加入幫忙！');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (splash) {
        // Keep splash for at least 1.8 seconds for a premium feel
        setTimeout(() => {
