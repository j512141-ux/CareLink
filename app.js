...
function renderAllSharePosts() {
    ...
    if (allList.children.length === 0) { 
        const otherPosts = [
            {
                img: 'https://images.unsplash.com/photo-1473116763249-2faaef81ccda?auto=format&fit=crop&q=80&w=800',
                name: '王奶奶',
            ...
            {
                img: 'https://images.unsplash.com/photo-1545239351-1141bd82e8a6?auto=format&fit=crop&q=80&w=800',
                name: '陳阿公',
...
function resetSharewallSubTabs() {
    ...
    // Clear sub-nav active states since we are showing the default feed
    const diaryNav = document.querySelector('#page-diary .sub-nav');
    if (diaryNav) {
        diaryNav.querySelectorAll('.sub-nav-item').forEach(item => item.classList.remove('active'));
    }
}
...
if (document.readyState !== 'loading') {
    const myShareTab = document.querySelector('.sub-nav-item[onclick*="sharewall-me"]');
    if (myShareTab) switchSubTab('sharewall-me', myShareTab, 'sharewall-section');
} else {
